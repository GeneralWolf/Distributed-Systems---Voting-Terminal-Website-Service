...
<body>
		...
		<table style="border-spacing:0 15px;"> 
			<tr> 
				...
				<c:if test="${userRole eq 'Admin' && election.status eq 'YetToStart'}">
					<input id="btnEdit1" type="button" value="Edit" onclick="onEditClick('title');"/>
			...
			<tr>
				...
				<c:forEach items="${election.lists}" var="list" varStatus="loop">
					<tr>
						...
						<c:if test="${election.status eq 'Done'}">
							<td> <label>${list.users.size()} &nbsp; votes </label></td>
						</c:if>
	...
	<div style="width:300px;float:left;">
		...
		<c:if test="${election.status eq 'Ongoing'}">
			<br/>
